...
const PRELIMINARY_RESULTS: ParticipantInfo[] = [
  { 순번: 1, 분야: "국악", 소속: "일신초등학교", 팀명: "구각와니", 신청자명: "장○완" },
...
export const ResultPage: FC = () => {
  const groupedResults = PRELIMINARY_RESULTS.reduce(
    (acc, participant) => {
      ...
      <div className="max-w-4xl w-full flex flex-col gap-8 py-6">
        ...
        <div className="bg-white p-8 mobile:p-6 shadow-sm border">
          <header className="flex flex-col justify-center items-center gap-8 mb-24">
            <div className="text-title4b mobile:text-body1b text-center text-gray-900">
              2025 광탈페 예선전 진출팀
            </div>
            <p className="text-body2r mobile:text-caption1r text-gray-500">
            ...
            <Link
              href="/result/detail"
              className="text-body2b mobile:text-caption1b text-main-600 underline"
            >
              협의회 참석 관련 안내 &gt;
            </Link>
          </header>
          ...
          <section className="space-y-28 mt-12">
            {fieldOrder.map(field => {
              ...
              return <FieldSection key={field} field={field} participants={participants} />;
            })}
          </section>
        </div>
      </div>
