    ...
    <div className={`${className}`}>
      <div className="hidden md:flex flex-row py-28 justify-center gap-4">
        {ARROWS_DATA.map((arrow, index) => (
          <Arrow key={`desktop-arrow-${index}`} color={arrow.color} text={arrow.text} />
        ))}
      </div>

      <div className="flex md:hidden flex-col py-28 items-center gap-8">
        <div className="flex flex-row gap-4">
          <Arrow color={ARROWS_DATA[0].color} text={ARROWS_DATA[0].text} />
          <Arrow color={ARROWS_DATA[1].color} text={ARROWS_DATA[1].text} />
        </div>
        <div className="flex flex-row gap-4">
          <Arrow color={ARROWS_DATA[2].color} text={ARROWS_DATA[2].text} />
          <Arrow color={ARROWS_DATA[3].color} text={ARROWS_DATA[3].text} />
        </div>
      </div>
    </div>
  );
